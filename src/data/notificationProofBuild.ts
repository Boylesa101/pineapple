...
export const NOTIFICATION_PROOF_BUILD_VERSION = '2.2.4';
...
export function withNotificationProofTrip(snapshot: AppDataSnapshot, now = new Date()) {
  ...
  const proofTrip: Trip = {
    ...
    attributionText: 'Temporary 2.2.4 transport notification verification trip',
    attributionMeta: { source: 'fallback', sourceLabel: 'Temporary 2.2.4 transport notification verification trip' },
    coverImageUri: null,
    heroImageRemoteUrl: null,
    heroImageStatus: 'idle',
    notes:
      'Temporary seeded trip for Pineapple 2.2.4 transport notification verification. Remove after on-device lock-screen proof is confirmed.',
